# src/app/api/search 分层指南

## 目录职责
搜索 API，委派 server/modules/search 并保持查询可追踪。
...
## 测试要求
- 首选验证：src/tests/search-route.test.ts、src/tests/search-repository.test.ts
- 修改类型边界、路由响应或跨模块契约时，加跑 `bun run type-check`。
- 涉及公开文档、发布卫生或路径重排时，加跑公开残留和 secret 扫描。

## 内层覆盖条件
- 本文件只在 `src/app/api/search/` 及其子目录内生效，子目录 AGENTS.md 可以收紧规则。
