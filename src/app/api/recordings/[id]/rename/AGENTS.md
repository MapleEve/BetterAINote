# src/app/api/recordings/[id]/rename 分层指南

## 目录职责
录音手动重命名 API，复用 rename-shared 校验。

## 真实文件
- `route.ts`

## 下级入口
- `src/app/api/recordings/[id]/rename/auto/`
...
## 测试要求
- 首选验证：src/tests/rename-route.test.ts
- 修改类型边界、路由响应或跨模块契约时，加跑 `bun run type-check`。
- 涉及公开文档、发布卫生或路径重排时，加跑公开残留和 secret 扫描。

## 内层覆盖条件
- 本文件只在 `src/app/api/recordings/[id]/rename/` 及其子目录内生效，子目录 AGENTS.md 可以收紧规则。
