# src/app/api/recording-tags 分层指南

## 目录职责
录音标签 API，委派 recording-tags 模块处理读写。

## 真实文件
- `route.ts`

## 下级入口
- `[id]/AGENTS.md`：单个录音标签删除 API。
...
## 内层覆盖条件
- 本文件只在 `src/app/api/recording-tags/` 及其子目录内生效，子目录 AGENTS.md 可以收紧规则。
