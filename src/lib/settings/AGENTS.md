# src/lib/settings 分层指南

## 目录职责
设置默认值、校验、规范化和 body schema。

## 真实文件
- `defaults.ts`
- `number-normalization.ts`
- `service-url-settings.ts`
- `sync-settings.ts`
- `title-generation-settings.ts`
- `transcription-settings-body.ts`
- `transcription-settings.ts`
- `user-settings.ts`
- `validation.ts`
- `value-normalization.ts`
- `voscript-provider-settings.ts`
- `voscript-settings-body.ts`
- `voscript-settings.ts`
...
## 内层覆盖条件
- 本文件只在 `src/lib/settings/` 及其子目录内生效，子目录 AGENTS.md 可以收紧规则。
