# src/lib/transcription 分层指南

## 目录职责
转写格式、作业、provider、文本保护和词级产物。
...
- `transcript-text-guards.ts`
- `voice-transcribe-metadata.ts`
- `word-artifacts.ts`

## 下级入口
- `src/lib/transcription/providers/`
...
## 测试要求
- 首选验证：src/tests/transcription-jobs.test.ts、src/tests/word-artifacts.test.ts、src/tests/repetition-detection.test.ts
- 修改类型边界、路由响应或跨模块契约时，加跑 `bun run type-check`。
- 涉及公开文档、发布卫生或路径重排时，加跑公开残留和 secret 扫描。

## 内层覆盖条件
- 本文件只在 `src/lib/transcription/` 及其子目录内生效，子目录 AGENTS.md 可以收紧规则。
