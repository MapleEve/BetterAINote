# src/features/data-sources 分层指南

## 目录职责
数据源设置表单和 onboarding 数据源 hook。

## 真实文件
- `data-source-field-control.tsx`
- `use-data-sources-settings.ts`
- `use-onboarding-data-source.ts`
...
## 测试要求
- 首选验证：src/tests/frontend-data-sources-regression.test.ts、src/tests/data-sources-presentation.test.ts
- 修改类型边界、路由响应或跨模块契约时，加跑 `bun run type-check`。
- 涉及公开文档、发布卫生或路径重排时，加跑公开残留和 secret 扫描。

## 内层覆盖条件
- 本文件只在 `src/features/data-sources/` 及其子目录内生效，子目录 AGENTS.md 可以收紧规则。
