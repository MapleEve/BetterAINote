# src/features/settings 分层指南

## 目录职责
前端设置 store 与设置页面组合层。

## 真实文件
- `display-settings-store.ts`
- `playback-settings-store.ts`
- `sync-settings-store.ts`
- `title-generation-settings-store.ts`
- `transcription-settings-store.ts`
- `voscript-settings-store.ts`
...
## 测试要求
- 首选验证：src/tests/settings-section-routes.test.ts、各 settings-store 测试
- 修改类型边界、路由响应或跨模块契约时，加跑 `bun run type-check`。
- 涉及公开文档、发布卫生或路径重排时，加跑公开残留和 secret 扫描。

## 内层覆盖条件
- 本文件只在 `src/features/settings/` 及其子目录内生效，子目录 AGENTS.md 可以收紧规则。
