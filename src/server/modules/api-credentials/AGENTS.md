# src/server/modules/api-credentials 分层指南

## 目录职责
服务端凭据模块，处理默认转写、私有转写和标题生成凭据。

## 真实文件
- `default-transcription.ts`
- `private-transcription.ts`
- `store.ts`
- `title-generation.ts`
...
## 内层覆盖条件
- 本文件只在 `src/server/modules/api-credentials/` 及其子目录内生效，子目录 AGENTS.md 可以收紧规则。
