# src/server/modules/recordings 分层指南

## 目录职责
录音服务端核心模块，覆盖列表、读模型、音频、重命名、来源报告、说话人、转写状态。

## 真实文件
- `audio.ts`
- `auto-rename-recording.ts`
- `delete-recording.ts`
- `index.ts`
- `list.ts`
- `ownership.ts`
...
## 内层覆盖条件
- 本文件只在 `src/server/modules/recordings/` 及其子目录内生效，子目录 AGENTS.md 可以收紧规则。
