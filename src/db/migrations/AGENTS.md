# src/db/migrations 分层指南

## 目录职责
Drizzle 迁移目录，记录每个业务域的基线 SQL 与 journal。
...
- `src/db/migrations/library/`
- `src/db/migrations/search/`
...
## 依赖方向
依赖方向是 db schema、paths、migrations 被 server 和测试使用；禁止在 UI 层直接访问数据库。

## 状态和数据流
状态和数据流以 schema、迁移和路径配置表达；迁移必须能从空库和已有库重复验证。
...
## 内层覆盖条件
- 本文件只在 `src/db/migrations/` 及其子目录内生效，子目录 AGENTS.md 可以收紧规则。
