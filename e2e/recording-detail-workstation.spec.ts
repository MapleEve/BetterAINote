...
function sourceReportDetailLoadedSubStatesMarkdown(
    ...
    return [
        ...
        "",
        `- Status: ${evidence.result}; Source report row remains ${evidence.scope.rowStatus}.`,
...
test("recording detail source report loaded sub-states match SOT pixels", async ({
    ...
    try {
        ...
        const evidence: SourceReportDetailLoadedSubStatesEvidence = {
            ...
            generatedAt: new Date().toISOString(),
            residualBoundaries: [
                "Source report matrix row intentionally remains PARTIAL.",
                "This slice covers only recording-detail loaded sub-states transcript-missing, summary-missing, and both-missing.",
                "No detail IAB all-state claim is made.",
                "No destructive/external open-source or repull click claim is made; controls are asserted for availability only.",
                "Broader all-page/all-control acceptance remains pending.",
            ],
            result,
            scope: {
